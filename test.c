...
void decode_packet(unsigned char *packet, int length) {
    ...
    unsigned short hx = (packet[4] << 8) | packet[5];
    unsigned short gx = (packet[6] << 8) | packet[7];
    unsigned short hy = (packet[8] << 8) | packet[9];
    unsigned short gy = (packet[10] << 8) | packet[11];
    unsigned short hz = (packet[12] << 8) | packet[13];
    unsigned short gz = (packet[14] << 8) | packet[15];
    unsigned short temperature = (packet[16] << 8) | packet[17];
    unsigned short voltage = (packet[18] << 8) | packet[19];
    
    printf("Decoded Data:\n");
    printf("Magnetic_X: %.4f\n", hx / 10000.0);
    printf("Acceleration_X: %.4f\n", gx / 10000.0);
    printf("Magnetic_Y: %.4f\n", hy / 10000.0);
    printf("Acceleration_Y: %.4f\n", gy / 10000.0);
    printf("Magnetic_Z: %.4f\n", hz / 10000.0);
    printf("Acceleration_Z: %.4f\n", gz / 10000.0);
